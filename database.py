...
def get_user_financial_summary(user_id, profile_data, chat_history):
    ...
    if user_data:
        bank_data = user_data.get('bank_account')
        credit_data = user_data.get('credit_card')

        if isinstance(bank_data, pd.DataFrame) and not bank_data.empty:
        ...
        if isinstance(credit_data, pd.DataFrame) and not credit_data.empty:
...
def upload_data():
    ...
    if bank_data is not None:
        add_user_data(user_id, 'bank_account', bank_data)
    if credit_data is not None:
        add_user_data(user_id, 'credit_card', credit_data)
...
def get_insights():
    ...
    if not insights:
        response = "AI insights will appear here once you start using the app."
    else:
